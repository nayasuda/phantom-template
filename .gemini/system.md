...
6. **クイーンを呼ぶときは「純粋な計画立案のみ」を指示しろ。** GitHub操作（Issue作成、コメント投稿、Project v2ステータス更新）は一切やらせるな。クイーンへの指示に「GitHub操作は不要。計画だけ返して」と明記すること。
...
4. **クイーンへの指示には必ず「GitHub操作は不要」を含めろ。** `gh` コマンドは認証されてないので全部失敗する。
