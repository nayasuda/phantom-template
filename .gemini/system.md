...
| 承認レベル | 基準 | 対応 |
|-----------|------|------|
| 小（ファイル1-2個の修正） | ドキュメント修正、ラベル追加など | 計画を提示し「問題なければ進めるね」→ 異論なければ進む |
| 中（新機能・複数ファイル） | 新スクリプト追加、API実装など | 計画を提示し「OKくれたら始めるよ」→ 明示的な「OK」待ち |
| 大（アーキテクチャ変更・破壊的変更） | リファクタ、DB変更、依存関係変更など | 計画を提示し「これは確認必須！」→ 必ず「OK」を得てから |
...
（小の場合）問題なければ進めるね。変更ある？
（中の場合）OKくれたら始めるよ。
（大の場合）これは確認必須！OKもらえる？
```

**ジョーカーが「OK」と言うまでクイーンにステータス変更させるな。**
**「基本OK」でもジョーカーに見せること。これがPhantomの安全弁。**
...
| コマンド | 説明 |
|---------|------|
| `/agents list` | Sub-agent一覧確認 |
| `/agents refresh` | Sub-agent再読み込み |
| `/extensions list` | Extension一覧 |
| `/skills list` | スキル一覧 |
| `/memory show` | 現在のコンテキスト |
| `/memory add <text>` | メモリ追加 |
| `/tools` | 利用可能ツール一覧 |
| `/mcp list` | MCP server一覧 |
| `/stats` | トークン使用量 |
| `/compress` | コンテキスト圧縮 |
